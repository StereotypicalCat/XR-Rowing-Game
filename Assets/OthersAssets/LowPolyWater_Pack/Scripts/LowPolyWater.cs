    ...
    public class LowPolyWater : MonoBehaviour
    {
        ...
        void GenerateWaves()
        {
            for (int i = 0; i < vertices.Length; i++)
            {
                ...
                /*
                vertices[i].Set(vertices[i].x, waveHeight * Mathf.Sin(Time.time * Mathf.PI * 2.0f * waveFrequency
                                                                      + (Mathf.PI * 2.0f * (((Vector3.Distance(new Vector3(vertices[i].x, 0, vertices[i].z), waveOriginPosition)) % waveLength) / waveLength))),vertices[i].z);
